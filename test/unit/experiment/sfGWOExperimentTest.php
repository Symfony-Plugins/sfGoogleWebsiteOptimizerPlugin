...
$test_page2 = array('module' => 'test2_module', 'action' => 'test2_action');
$test_param = array(
  'key'   => '123123123',
  'uacct' => 'XX-XXXXX-X',
...
$context->request->initialize($context);

// test associative array of page params in config
$context->request->setParameter('module', $test_page1['module'].'oops');
$context->request->setParameter('action', $test_page1['action']);
$t->ok(!$experiment->connect($context->request), 'assoc no connection ok');

$context->request->setParameter('module', $test_page1['module']);
$t->ok($experiment->connect($context->request), 'assoc connection ok');
$t->is_deeply(
...
$context->request->setParameter('module', $test_page2['module']);
$context->request->setParameter('action', $test_page2['action'].'oops');
$t->ok(!$experiment->connect($context->request), 'indexed no connection ok');

$context->request->setParameter('action', $test_page2['action']);
$t->ok($experiment->connect($context->request), 'indexed connection ok');
$t->is_deeply(
  $experiment->getParameterHolder()->getAll('connection'), 
